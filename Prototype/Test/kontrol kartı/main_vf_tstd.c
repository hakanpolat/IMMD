...
#define switching_frequency 50000	// Hz
...
int main(void) {
	...
	ConfigCpuTimer(&CpuTimer1, 100, 50000000); //2 seconds
...
*/
    ...
    // This is set for vbyf test on EVM
    //GpioCtrlRegs.GPAPUD.bit.GPIO18 = 0;   // enable pull-up on GPIO18 (for test)
    GpioCtrlRegs.GPAPUD.bit.GPIO18 = 0;   // enable pull-up on GPIO19 (EPWM10B)
    //GpioCtrlRegs.GPAGMUX2.bit.GPIO18 = 1;  // Configure GPIO18 as EPWM10A
    GpioCtrlRegs.GPAGMUX2.bit.GPIO18 = 0;  // Configure GPIO19 as EPWM10B
    //GpioCtrlRegs.GPAMUX2.bit.GPIO18 = 1;   // Configure GPIO18 as EPWM10A
    GpioCtrlRegs.GPAMUX2.bit.GPIO18 = 0;   // Configure GPIO19 as EPWM10B
...
__interrupt void epwm1_isr(void) {
	...
	EPwm1Regs.CMPA.half.CMPA = EPwm1Regs.TBPRD * epwm8_dutycycle; // Module-4 Phase-A PWM (ePWM1)
	EPwm2Regs.CMPA.half.CMPA = EPwm2Regs.TBPRD * epwm7_dutycycle; // Module-2 Phase-C PWM (ePWM2)
	EPwm3Regs.CMPA.half.CMPA = EPwm3Regs.TBPRD * epwm6_dutycycle; // Module-2 Phase-B PWM (ePWM3)
		...
		*/
		...
		if (EQep1Regs.QEPSTS.bit.UPEVNT == 1) {
			...
			fAngularMechanicalSpeed =
					uiUpEventTickValue * ((CLOCKHZ / 128) / EQep1Regs.QCPRDLAT) * 2 * PI;/*TODO; parameterize the values,uiUpEventTickValue comes from UPPS value = 16 for now,128 comes from ccps value*/
			fAngularMechanicalSpeed = fAngularMechanicalSpeed / (float32) (ENCODERMAXTICKCOUNT);
...
void Setup_EQEP(void) {
	...
	EQep1Regs.QCAPCTL.bit.CCPS = 7; // eQEP capture timer clock prescaler: // CAPCLK = SYSCLKOUT/128
	EQep1Regs.QCAPCTL.bit.UPPS = 0x4; // Unit position event prescaler: UPEVNT = QCLK/16 , QCLK is triggered in every rising or falling edge of A or B
