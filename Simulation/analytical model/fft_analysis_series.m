...
%% DC link voltage - M1
vdc1_FFTDATA = power_fftscope(vdc1);
vdc1_FFTDATA.startTime = fft_start;
vdc1_FFTDATA.cycles = fft_cycle; 
vdc1_FFTDATA.fundamental = fft_fund; 
vdc1_FFTDATA.maxFrequency = fft_maxfreq;
vdc1_FFTDATA.THDmaxFrequency = fft_THDmaxfreq;
vdc1_FFTDATA = power_fftscope(vdc1_FFTDATA);
num = numel(vdc1_FFTDATA.mag);
vdc1_DCcomp = vdc1_FFTDATA.mag(1);
%clc;
fprintf('\n----- DC Link Voltage - M1 -----\n');
%fprintf('\nDC component: %g\n',vdc1_DCcomp);
for k = 1:num
    if (vdc1_FFTDATA.mag(k) >= vdc1_DCcomp*5e-4)
       fprintf('Freq: %g, Mag: %g, Phase: %g\n',vdc1_FFTDATA.freq(k),vdc1_FFTDATA.mag(k),vdc1_FFTDATA.phase(k));
    end    
end

%% DC link voltage
vdc2_FFTDATA = power_fftscope(vdc2);
vdc2_FFTDATA.startTime = fft_start;
vdc2_FFTDATA.cycles = fft_cycle; 
vdc2_FFTDATA.fundamental = fft_fund; 
vdc2_FFTDATA.maxFrequency = fft_maxfreq;
vdc2_FFTDATA.THDmaxFrequency = fft_THDmaxfreq;
vdc2_FFTDATA = power_fftscope(vdc2_FFTDATA);
num = numel(vdc2_FFTDATA.mag);
vdc2_DCcomp = vdc2_FFTDATA.mag(1);
%clc;
fprintf('\n----- DC Link Voltage - M2 -----\n');
%fprintf('\nDC component: %g\n',vdc2_DCcomp);
for k = 1:num
    if (vdc2_FFTDATA.mag(k) >= vdc2_DCcomp*5e-4)
       fprintf('Freq: %g, Mag: %g, Phase: %g\n',vdc2_FFTDATA.freq(k),vdc2_FFTDATA.mag(k),vdc2_FFTDATA.phase(k));
    end    
end
