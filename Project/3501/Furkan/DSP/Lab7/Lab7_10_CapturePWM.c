...
void main(void)
{
	...
	ConfigCpuTimer(&CpuTimer0, 150, 100000); //Set CPU Timer-0 overflows @ 100 milliseconds
	                                        // Here 150 stands for 150 MHz, 100000 for 100 msecs
	PieCtrlRegs.PIEIER1.bit.INTx7 = 1; // Enable CPU Timer 0 INT
	PieCtrlRegs.PIEIER4.bit.INTx1 = 1; // Enable ECAP1_INT in Pie 4
	IER |= 0x0009; // Enable Interrupt Core Line
...
void Setup_ePWM1A(void)
{
    EPwm1Regs.TBCTL.bit.CLKDIV = 0; //010 -> /4
    EPwm1Regs.TBCTL.bit.HSPCLKDIV = 1; // 101 -> /10
    EPwm1Regs.TBCTL.bit.CTRMODE = 2; // both up and down counting
    EPwm1Regs.TBPRD = 37500; // 1875 Cycle will be counted
...
interrupt void eCAP1_isr(void)
{
  ECap1Regs.ECCLR.bit.INT = 1; // isr visit declared
  ECap1Regs.ECCLR.bit.CEVT3 = 1; // CAP3 interrupt is re-enabled.
